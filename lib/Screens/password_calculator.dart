...
String calculatePassword(String email, String keyword) {
  int len = email.length + 1; //Gets length of the email + 1 to avoid zeros
  int asc = email.codeUnitAt(0) +
      1; //Gets ascii value of first digit of email + 1 to avoid zeros
  int mult = len * asc; //Multiply to get a larger value
  String strMult = mult.toString(); //convert to string
  strMult = strMult +
      strMult +
      strMult +
      strMult; //concat to guarantee its at least 4 digits
  ...
  password = password.substring(0, sha_first_ascii) +
      capital_letter +
      password.substring(sha_first_ascii);
  password = password.substring(0, sha_second_ascii) +
      special_char +
      password.substring(sha_second_ascii);
